...
sample10()->
   io:format( astExtract:transCodeStr("-module(test). -compile(export_all). f()->1.")).

sample11()->
    io:format( 
      astExtract:transCodeStr("-module(test)."++ 
			   "-compile(export_all)."++
			   " f()-> if true -> 3 end."
			  )).
...
sample14()->
    io:format( 
      astExtract:transCodeStr("-module(test).\n"++ 
			   "-compile(export_all).\n"++
			   " f(X) when X>1 -> if X>2,X<4 -> 2; true -> 3 end."
			  )).
